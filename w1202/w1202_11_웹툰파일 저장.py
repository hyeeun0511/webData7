...
print('[ 실시간 신작 랭킹 ]')
for i,li in enumerate(lis):  # index 번호가 넘어옴
    print(li.find("img")['src'])  # 이미지
    # request url정보를 가져옴. jpg -> jpg정보를 가져옴
    img_req = requests.get(li.find("img")['src'],headers=headers)   # requests.get(li.find("img")['src']) 이 html정보를 img_req 에 담음
    # headers=headers 넣어야 이미지를 다운받을 수있음
    os.makedirs("webtoon",exist_ok=True)  # 폴더가 없을때만 생성됨   # 있으면 넘어가고 없으면 만들라는 의미
    # if not os.isdir("webtoon"):   # 원래는 if문으로 하는데 위에처럼 한줄로 해도 됨
    #      os.makedirs("webtoon")  # 폴더가 없을때만 생성됨
    # 파일저장  (실행을 시켜야 webtoon파일 생성됨)
    with open(f"webtoon/webtoon_{i+1}.jpg","wb") as f:  # 저장하려면 w / 파일을 저장하려면 wb
        f.write(img_req.content)  # 파일로 저장
    print(li.find("strong",{"class":"AsideList__ranking--sNPZy"}).text.strip())  # 랭킹
    print(li.find("span",{"class":"text"}).text.strip())  # 제목
    print(li.find("a",{"class":"ContentAuthor__author--CTAAP"}).text.strip())  # 작가
    print("-"*50)
